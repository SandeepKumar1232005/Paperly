...
export const api = {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const user: User = {
          ...
          avatar: resolveMediaUrl(userData.avatar) || `https://api.dicebear.com/7.x/avataaars/svg?seed=${userData.email}`,
          ...
          handwriting_samples: (userData.handwriting_samples || []).map((s: any) => 
        ...
        if (localUserIdx !== -1) {
          ...
          // Preserve local fields if missing on backend, otherwise prefer backend
          user.qr_code_url = user.qr_code_url || localUser.qr_code_url;
          user.handwriting_samples = (user.handwriting_samples && user.handwriting_samples.length > 0) ? user.handwriting_samples : localUser.handwriting_samples;
          user.pricePerPage = user.pricePerPage || localUser.pricePerPage;
      ...
      if (response.ok) {
        ...
        if (idx !== -1) {
          ...
          const backendUser = {
            ...
            avatar: resolveMediaUrl(data.avatar) || updates.avatar || users[idx].avatar,
            ...
            qr_code_url: resolveMediaUrl(data.qr_code_url) || updates.qr_code_url || users[idx].qr_code_url,
  ...
  async getWriters(coords?: { lat: number; lon: number }): Promise<User[]> {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const writers: User[] = data.map((u: any) => {
          const fullName = u.name || [u.first_name, u.last_name].filter(Boolean).join(' ') || u.username || 'User';
          return {
            id: String(u.id),
            name: fullName,
            email: u.email,
            role: 'WRITER' as const,
            avatar: resolveMediaUrl(u.avatar) || `https://api.dicebear.com/7.x/avataaars/svg?seed=${u.email}`,
            lastActive: new Date().toISOString(),
            address: u.address || '',
            is_verified: u.is_verified || false,
            handwriting_style: u.handwriting_style,
            handwriting_confidence: u.handwriting_confidence,
            handwriting_samples: (u.handwriting_samples || []).map((s: any) => 
              resolveMediaUrl(typeof s === 'string' ? s : s.sample_url)
            ),
            availability_status: u.availability_status || 'ONLINE',
            coordinates: u.coordinates,
            qr_code_url: resolveMediaUrl(u.qr_code_url),
