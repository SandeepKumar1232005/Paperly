...
export function Writers({ onNavigate, onHire, currentUser }: WritersProps) {
    ...
    const filteredWriters = writers.filter(writer => {
        // 1. Handwriting Style Filter (Case-insensitive)
        if (handwritingFilter !== 'All') {
            const writerStyle = (writer.handwriting_style || '').toLowerCase();
            const filterStyle = handwritingFilter.toLowerCase();
            if (writerStyle !== filterStyle) return false;
        }

        // 2. Location Filtering (Optional/Relaxed)
        // We only filter by address if both have addresses set, 
        // and we don't block the result if the student hasn't set an address yet.
        const studentAddr = (currentUser?.address || '').toLowerCase().trim();
        const writerAddr = (writer.address || '').toLowerCase().trim();

        if (studentAddr && writerAddr) {
            const isMatch = writerAddr.includes(studentAddr) || studentAddr.includes(writerAddr);
            // If we want to strictly show only local writers, we would return isMatch
            // But for now, let's just prioritize it or keep it optional.
            // To fix the user's "No Writers Found" issue, we'll disable the hard block.
            // return isMatch; 
        }
