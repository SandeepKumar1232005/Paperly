...
const WriterProfileModal: React.FC<WriterProfileModalProps> = ({ writer, onClose, onHire }) => {
  ...
  // Close on Escape key & scroll lock
  useEffect(() => {
    // Only handle scroll lock for lightbox here, as Modal handles its own scroll lock
  ...
  return (
    <Modal
      ...
      bodyClassName="p-0 overflow-y-auto overflow-x-hidden flex-1"
      headerContent={
    ...
    >
      ...
      {/* Sticky Footer */}
      <div className="p-6 border-t border-[var(--border)] flex gap-3 flex-shrink-0 sticky bottom-0 z-50 bg-[var(--bg-secondary)]">
        <motion.button
